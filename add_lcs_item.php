<?php
session_start();
// Redirect to login if not logged in (must run before any output)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    if ($del = $conn->prepare("DELETE FROM lcs_item WHERE id = ?")) {
        $del->bind_param('i', $del_id);
        $del->execute();
    }
    header("Location: add_lcs_item.php?message=deleted");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['delete_id'])) {
    ...
    if ($item_name !== '') {
        ...
        $check = $conn->prepare("SELECT COUNT(*) FROM lcs_item WHERE LOWER(item_name) = LOWER(?)");
        ...
        if ($count > 0) {
            // Duplicate item
            header("Location: add_lcs_item.php?message=exists");
            exit();
        } else {
            // Add new item
            $stmt = $conn->prepare("INSERT INTO lcs_item (item_name, description, is_active) VALUES (?, ?, ?)");
            $stmt->bind_param("ssi", $item_name, $description, $is_active);
            $stmt->execute();
            header("Location: add_lcs_item.php?message=created");
            exit();
        }
    }
}
...
if (isset($_GET['message'])) {
    switch ($_GET['message']) {
        case 'created':
            $message = "✅ LCS Item added successfully!";
            $type = "success";
            break;
        case 'exists':
            $message = "⚠️ This LCS Item already exists!";
            $type = "error";
            break;
        case 'deleted':
            $message = "🗑️ LCS Item deleted successfully!";
            $type = "success";
            break;
    }
}
...
$items = [];
$res = $conn->query("SELECT id, item_name, description, is_active FROM lcs_item ORDER BY id DESC");
if ($res) {
...
<body>
	...
	<div class="container">
		<div class="card">
			<h2 style="text-align:center; color:#2d3748;">🧩 LCS Items Master</h2>
			<p style="text-align:center; color:#718096;">Create and manage LCS items used in daily status.</p>
			                ...
			                <tr>
			                    ...
			                    <td style="padding: 8px; border-bottom: 1px solid #eee;">
			                        <div style="display:flex; gap:4px;">
			                            <a href="edit_lcs_item.php?item_id=<?php echo (int)$it['id']; ?>" 
