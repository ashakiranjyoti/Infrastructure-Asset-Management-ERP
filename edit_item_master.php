...
// Load item
$stmt = $conn->prepare("SELECT id, item_name, description, is_active FROM items_master WHERE id = ?");
if (!$stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_name !== '') {
        ...
        $check = $conn->prepare("SELECT COUNT(*) FROM items_master WHERE LOWER(item_name) = LOWER(?) AND id != ?");
        ...
        if ($count > 0) {
            $error = "⚠️ Another item with the same name already exists!";
        } else {
            $up = $conn->prepare("UPDATE items_master SET item_name = ?, description = ?, is_active = ? WHERE id = ?");
            if (!$up) {
                $error = 'Database error: ' . $conn->error;
            } else {
                $up->bind_param('ssii', $item_name, $description, $is_active, $item_id);
                if ($up->execute()) {
                    header('Location: add_item_master.php?message=updated');
                    exit();
                } else {
...
<head>
    ...
    <title>Edit Item - <?php echo htmlspecialchars($item['item_name']); ?></title>
...
<body>
	...
	<div class="container">
		<div class="card" style="max-width:800px; margin:0 auto;">
			<h2 style="text-align:center; color:#2d3748;">✏️ Edit Item</h2>
			<p style="text-align:center; color:#718096;">Modify details for the selected item.</p>
			...
			<form method="POST" class="parameter-form" style="max-width:700px; margin:0 auto;">
				...
				<div class="btn-group">
					<a href="add_item_master.php" class="btn btn-secondary">Cancel</a>
